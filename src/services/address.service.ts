...
export class AddressService {
  ...
  public async getAddressByCustomer(userId: string) {
    ...
    if (user?.customer.address.length === 0)
      throw new AppError('No tiene direcciones registradas', NOT_FOUND);
  ...
  public async createAddress(createAddressDto: AddressDTO, userId: string) {
    ...
    if (!user.customer)
      throw new AppError('Cliente no tiene un registro', NOT_FOUND);
  ...
  public async deleteAddress(addressId: string, userId: string) {
    ...
    if (result.affected === 0) {
      throw new AppError(
        'Falló al eliminar la dirección',
